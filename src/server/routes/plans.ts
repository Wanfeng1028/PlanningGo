    ...
    if (!db) {
      ...
      if (useActions && useActions.length > 0) {
        saveActions(useActions.map((a) => ({
          ...
          userId,
          type: a.type as ExecutionAction["type"],
          provider: ((a.provider as string) || "mock") as ExecutionAction["provider"],
          status: normalizeActionStatus(a.status) as ExecutionAction["status"],
          title: a.title,
